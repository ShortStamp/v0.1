...
export function ConflictBadge({ compat, resolveName, position = "above", debugMode = false }: ConflictBadgeProps) {
  ...
  // Pick the most descriptive reason to show
  const displayReason = compat.reason ||
    (compat.reasons && compat.reasons.length > 0 ? compat.reasons[0].text : "");
  ...
  return (
    <>
      ...
      {open && createPortal(
        <div
          ref={panelRef}
          className="fixed z-[9999] w-72 rounded-xl border border-border/50 bg-white p-4 shadow-2xl"
          style={{
            top: coords.top,
            left: coords.left,
        ...
        >
          {/* Conflicting products — only show ones we can actually name */}
          {(() => {
            const named = compat.conflictingProductIds
              .map((id) => resolveName(id))
              .filter((n) => n && n.toLowerCase() !== "another product")
              .slice(0, 2);
            if (named.length === 0) return null;
            return (
              <p className="mb-2 text-[9px] font-bold text-foreground/50 font-sans uppercase tracking-widest">
                Conflicts with {named.join(", ")}
              </p>
            );
          })()}
          ...
          {displayReason && (
            <p className="text-[11px] leading-relaxed text-foreground font-sans">
              {displayReason}
            </p>
          )}
...
export function PassBadge({ trace, position = "above" }: PassBadgeProps) {
  return (
    <span className="rounded-full bg-green-50 px-2 py-1 text-[8px] font-bold uppercase tracking-[0.1em] text-green-600 border border-green-100 font-sans">
      Compatible
    </span>
  );
}
