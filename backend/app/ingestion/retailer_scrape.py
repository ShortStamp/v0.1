...
def _amazon_bestseller_candidates(html: str) -> list[ScrapedCandidate]:
    ...
    for match in anchor_pattern.finditer(html):
        ...
        name = _clean_product_name(image_match.group("alt")) if image_match else _guess_name_from_url(url, "amazon")
        image_url = image_match.group("src") if image_match else None
        seen.add(asin)
        candidates.append(
            ScrapedCandidate(
                external_id=asin,
    ...
    *,
    ...
    if product is None:
        ...
        if candidate_brand_norm and candidate_brand_norm not in generic_brand_values:
            product.brand_id = brand_id
        if candidate.image_url and product.image_url == "/placeholder-product.jpg":
